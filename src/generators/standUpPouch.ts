import { Dimensions, PrintSettings, Colors } from '../types'
import { SVGBuilder } from '../utils/svgBuilder'

export function generateStandUpPouch(
  dimensions: Dimensions,
  settings: PrintSettings,
  colors: Colors
): string {
  const builder = new SVGBuilder()
  const { width, height, depth } = dimensions
  const { bleed, safeZone, glueTab } = settings

  const totalWidth = width + depth * 2 + glueTab + bleed * 2
  const totalHeight = height + depth + bleed * 2

  // Bleed box
  builder.addRect(
    0,
    0,
    totalWidth,
    totalHeight,
    { stroke: colors.bleed, fill: 'none', strokeWidth: 0.5, strokeDasharray: '2,2' }
  )

  const startX = bleed
  const startY = bleed

  // Left gusset
  builder.addRect(startX, startY, depth, height + depth, {
    stroke: colors.cut,
    fill: 'none',
    strokeWidth: 1,
  })

  // Front panel
  builder.addRect(startX + depth, startY, width, height, {
    stroke: colors.cut,
    fill: 'none',
    strokeWidth: 1,
  })

  // Right gusset
  builder.addRect(startX + depth + width, startY, depth, height + depth, {
    stroke: colors.cut,
    fill: 'none',
    strokeWidth: 1,
  })

  // Glue tab
  builder.addRect(startX + depth * 2 + width, startY, glueTab, height, {
    stroke: colors.cut,
    fill: 'none',
    strokeWidth: 1,
    strokeDasharray: '2,2',
  })

  // Fold lines
  builder.addLine(startX + depth, startY, startX + depth + width, startY, {
    stroke: colors.fold,
    strokeWidth: 0.5,
    strokeDasharray: '1,1',
  })
  builder.addLine(
    startX + depth,
    startY + height,
    startX + depth + width,
    startY + height,
    { stroke: colors.fold, strokeWidth: 0.5, strokeDasharray: '1,1' }
  )

  return builder.toSVG()
}
